...
var debug = {
    ...
    getOriWindow: function () {
        return parent.datadevInit.getActiveWindow(getKey($("#gitProjectId").val(), debug.oriFilePath));
    },
    showDebug: function () {
        parent.$.removeMsg();
    ...
    changeBreakPointStatus: function (breakPointsStatus) {
        ...
        if (!debug.oriFilePath || debug.oriFilePath && debug.oriFilePath == $("#gitProjectFilePath").val()) {
            ...
            for (var index = 0; index < dependencyPaths.length; index++) {
                var realWindow = parent.datadevInit.getActiveWindow(getKey($("#gitProjectId").val(), dependencyPaths[index]));
    ...
    close: function () {
        ...
        debug.broadcast(keys, function (realWindow) {
            ...
            parent.resetAllTopButton(realWindow);
            jq("#debugInfo").remove();
    ...
    broadcast: function (keys, operate) {
        if (keys) {
            for (var index = 0; index < keys.length; index++) {
                var realWindow = parent.datadevInit.getActiveWindow(getKey($("#gitProjectId").val(), keys[index]));
    ...
    getBreakPoints: function (scriptFilePath) {
        ...
        var realWindow = parent.datadevInit.getActiveWindow(scriptKey);
    ...
    getDependencyPaths: function () {
        ...
        var dependencys = parent.debugDepndendedMap.get(key);
    ...
    showCodeLine: function (messageObj, showError) {
        ...
        if (debug.status != debug.constant.EXCEPTION) {
            ...
            if (messageObj.data.scriptFilePath == debug.oriFilePath) {
                ...
                parent.openScript($("#gitProjectId").val(), debug.oriFilePath);
                var keys = debug.getDependencyPaths();
                debug.broadcast(keys, function (realWindow) {
                ...
                var dependencyWindow = parent.datadevInit.getActiveWindow(dependencykey);
                var transferObj = {};
                ...
                if (dependencyWindow && dependencyWindow.win && dependencyWindow.win.debug) {
                    parent.openScript($("#gitProjectId").val(), messageObj.data.scriptFilePath);
                    dependencyWindow.win.debug.initDependencyDebug(transferObj, false);
                } else {
                    parent.initFunctionMap.put(getKey(null, messageObj.data.scriptFilePath), function (key, win) {
                        var window = parent.datadevInit.getActiveWindow(getKey($("#gitProjectId").val(), messageObj.data.scriptFilePath));
    ...
    initDependencyDebug(statusObj, newOpen) {
        ...
        setRunMode(true);
        initMode("debug-mode");
        parent.disableAllTopButton && parent.disableAllTopButton(getKey());
        parent.parentInitTopButton && parent.parentInitTopButton(getKey(), parent.TOP_BUTTON.stop, true);
...
function shieldByVersionChange(isLeft) {
    if (isLeft) {
        ...
        parent.parentInitTopButton && parent.parentInitTopButton(getKey(), "git", false);
        parent.parentInitTopButton && parent.parentInitTopButton(getKey(), "upLine", false);
        parent.parentInitTopButton && parent.parentInitTopButton(getKey(), "dependencyButton", false);
        ...
        $("#update2NewVersion").show();
        parent.parentInitTopButton && parent.parentInitTopButton(getKey(), "git", $("#filePosition").val() != "target");
        parent.parentInitTopButton && parent.parentInitTopButton(getKey(), "upLine", true);
        parent.parentInitTopButton && parent.parentInitTopButton(getKey(), "dependencyButton", $("#isShow").val() != 1 && ($("#scriptType").val() == 2 || $("#scriptType").val() == 3));
...
function command2run() {
    parent.parentButtonClick && parent.parentButtonClick("run");
}

//保存
function command2save() {
    parent.parentButtonClick && parent.parentButtonClick("save");
}
...
function command2debug() {
    parent.parentButtonClick && parent.parentButtonClick("debug");
}

//停止
function command2stop(node) {
    if (!node || !node.hasClass("ico-disabled")) {
        parent.stopScript && parent.stopScript(debug.getOriWindow());
...
function command2restart() {
    if (getRunMode() == 1) {
        ...
        //run restart
        parent.restartScript(getKey(), $("#scriptRunDetailId").val());
    }
    parent.parentButtonClick && parent.parentButtonClick("stop");
}
...
function command2disabled() {
    ...
    var windows = parent.datadevInit.getAllWindow();
...
function hideMoreFunction(fileMd5) {
    $(".moreFunction").hide();
}
...
function showMorefunction(event) {
    if (event.which == 3) {
        ...
        if (parent.getIframeOffset && parent.getWindowSize) {
            var offset = parent.getIframeOffset();
            var size = parent.getWindowSize();
            if (offset && size) {
...
function initLocation() {
    if ($("#isShow").val() == 1) {
        ...
        parent.parentInitTopButton && parent.parentInitTopButton(getKey(), "dependencyButton", !isTemplate() && ($("#scriptType").val() == 2 || $("#scriptType").val() == 3));
        parent.parentInitTopButton && parent.parentInitTopButton(getKey(), "upLine", !isTemplate() && ($("#scriptType").val() == 1 || $("#scriptType").val() == 2 || $("#scriptType").val() == 3 || $("#scriptType").val() == 4));
    ...
    if ($("#isShow").val() == 1 || $("#filePosition").val() == "target" || !initContentStatus) {
        ...
        parent.parentInitTopButton && parent.parentInitTopButton(getKey(), "git", !isTemplate());
    }
}
    ...
    if ($("#canEdit").val() && $("#canEdit").val() == "true") {
        commonAjaxEvents.commonPostAjax(getScriptUrl, {
            ...
            if (data) {
                ...
                parent.refreshFileInfoStatus && parent.refreshFileInfoStatus(gitProjectId, gitProjectFilePath, md5, null, gitStatus);
                editor.setValue(res);
                editor.focus();
                editor.clearSelection()
...
function initIframeKeyMap() {
    ...
    $(document).keydown(function (event) {
        if (event.ctrlKey && event.shiftKey && event.keyCode == 13) {
            ...
            parent.saveScript && parent.saveScript();
            return false;
        } else if (event.metaKey && event.keyCode == 83) {
            parent.saveScript && parent.saveScript();
            return false;
        } else if (event.ctrlKey && event.keyCode == 13) {
            parent.runOrStopScript && parent.runOrStopScript();
            return false;
...
function renameScript() {
    ...
    parent.parentRenameScript && parent.parentRenameScript(gitProjectId, gitProjectFilePath, gitProjectDirPath, name, type);
}
...
function initCallBack() {
    ...
    var callBack = parent.afterInitCallBackMap && parent.afterInitCallBackMap.get(key);
    parent.afterInitCallBackMap && parent.afterInitCallBackMap.put(key, undefined)
    callBack && callBack();
}
...
function initTopButton() {
    ...
    parent.parentInitTopButtons && parent.parentInitTopButtons(key, scriptMapParams);
}
...
function initFunction() {
    var fun = parent.initFunctionMap.get(getKey());
    fun && fun(getKey(), window);
    parent.initFunctionMap.remove(getKey());
}
...
$(function () {
    ...
    function initInfo() {
        var key = getKey();
        if (parent && parent.scriptInitStatusMap && parent.scriptInitStatusMap.get(key)) {
            parent.scriptInitStatusMap.get(key)();
            parent.scriptInitStatusMap.remove(key);
        }
    ...
    function initConfigAndArgs() {
        parent.changeConfig && parent.changeConfig($("#configId").val(), getKey(), true)
    }
    ...
    function initEvent() {
        $(".moreFunction li").click(function () {
            ...
            if (enevntType === "addtemplate") {
                parent.saveTemplate && parent.saveTemplate(parent.datadevInit.getActiveWindow(getKey()));
            }
            if (enevntType === "down") {
                parent.downloadScript && parent.downloadScript($("#gitProjectFilePath").val(), $("#gitProjectId").val());
            }
        ...
        $("#rightClickGitMenu").on("click", "li", function () {
            ...
            if (liClass == "pull") {
                parent.pullFile && parent.pullFile($("#gitProjectId").val(), $("#gitProjectFilePath").val())
            } else if (liClass == "push") {
                parent.pushFile && parent.pushFile($("#gitProjectId").val(), $("#gitProjectFilePath").val())
            } else if (liClass == "showGitHistory") {
                parent.showGitHistory && parent.showGitHistory($("#gitProjectId").val(), $("#gitProjectFilePath").val(), false);
            }
        })
        //右键目录git
        ...
        $("#cheDepDiv").click(function () {
            parent.choDep && parent.choDep($('#gitProjectId').val(), $("#gitProjectFilePath").val());
        })
        ...
        $("#chooseVersionBtn").click(function () {
            parent.datadevInit && parent.datadevInit.mergeChoose($("#version").val() != $("#lastVersion").val(), verifyModify());
        })
    }
