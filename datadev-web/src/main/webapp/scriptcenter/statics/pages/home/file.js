                    ...
                    if (data && data.code == 0 && data.obj) {
                        ...
                        switch (modeCode) {
                            ...
                            case SAVE_MODE:
                                ...
                                var window = parent.datadevInit.getActiveWindow(newKey);
                                if(window){
                                    var editValue = window.editor.getValue();
                                    var newMd5 = $.md5(editValue);
                                    window.jq("#fileMd5").val(newMd5)
                                }
                                refreshStatus(newKey, zNode.version, zNode.gitProjectId, zNode.path, zNode.md5, zNode.parentPath, zNode.name);
                                if (modeCode != MOVE_MODE) {
                            ...
                            case SCRIPT_RENAME_MODE:
                                if (zNode.status == "1") {
                                    ...
                                    refreshScriptInifo(key, zNode.gitProjectId, zNode.path, zNode.name);
                                    var newKey = getKey(zNode.gitProjectId, zNode.path);
                                    refreshStatus(newKey, zNode.version, zNode.gitProjectId, zNode.path);
                                }
                                break;
                        }
