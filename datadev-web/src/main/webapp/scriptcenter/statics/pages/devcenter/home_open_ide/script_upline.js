...
$(function () {
    ...
    var applicationId = top.window.projectSpaceId;
    ...
    $("#uplineOk").click(function (event) {
        ...
        if (valid) {
            ...
            if (applicationId) {
                ...
                commonAjaxEvents.commonSpinPostAjax("/scriptcenter/devcenter/upLineScript.ajax", {
                    ...
                    $.bdpMsg({
                        title: "提示",
                        width: "350px",
                        mainContent: "<p style='margin-top: 30px;margin-bottom: 30px;'>脚本已提交发布申请，可进入<a href='/release/"+top.window.projectSpaceId+"/unRelease' target='_blank'>发布中心</a>查看审批状态，待审批完成后可点击“确认发布”按钮，将脚本发布至任务调度buffalo </p>",
                        buttons: [
                            {
                                text: "查看发布列表",
                                event: function () {
                                    window.open("/release/"+top.window.projectSpaceId+"/unRelease","_blank");
                                },
            ...
            commonAjaxEvents.commonPostAjax("/scriptcenter/buffalo/getJobNum.ajax", {
                ...
                if (data.obj.buffaloInfo) {
                    ...
                    if (!buffaloInfo.gitProjectId && !buffaloInfo.gitProjectFilePath) {
                        initTaskListJqgrid(applicationId);
                        ...
                        $.errorMsg("项目空间【" + data.obj.appGroupName + "】下有同名脚本，请修改脚本名称")
                    } else {
                        //已经同步过，并且是正确的脚本
                        initTaskListJqgrid(applicationId);
                    }
    ...
    $.fn.upLine = function (id, path, name, vers, type ) {
        ...
        $("#uplineAppSelect").val(top.window.projectSpaceName);
        initJobNum(function () {
            $("#upLineModal").modal("show");
        });
        ...
        // }, null, function (node, data) {
        //     var apps = data.obj;
        //     var currentProjectSpaceId = top.window.projectSpaceId;
        //     var options = "<option value=''></option>";
        //     if (apps && apps.length) {
        //         for (var index = 0; index < apps.length; index++) {
        //             var selected = apps[index].appgroupId === currentProjectSpaceId ? "selected='selected'" : "";
