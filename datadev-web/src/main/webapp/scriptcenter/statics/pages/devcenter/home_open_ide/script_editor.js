...
function modifyRunStatus(runDetailId, key) {
    ...
    //notify run success
    resetAllTopButton && resetAllTopButton(datadevInit.getActiveWindow(key));
            ...
            commonAjaxEvents.runTimeLogPostAjax(LOG_URI, {
                ...
                if (data.obj) {
                    ...
                    if (data.obj.isErrorLog + "" == "true") {
                        showAccountErrorByLog && showAccountErrorByLog(key);
                        return;
                    }
                ...
                var status = 0;
                if (runStatusMap) {
                    status = runStatusMap.get(runDetailId) || 0;
                }
                if (status == 0) {
                    window.setTimeout(function () {
