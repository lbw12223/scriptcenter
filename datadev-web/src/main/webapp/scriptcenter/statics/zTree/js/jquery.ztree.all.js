    ...
    var settings = {}, roots = {}, caches = {},
        ...
        _eventProxy = function (event) {
            ...
            if (tools.eqs(event.type, "mousedown")) {
                ...
                var nodeDiv = tools.getMDom(setting, target, [{tagName: "div", attrName: "tree_node_div"}]);
                target=nodeDiv||target;
                treeEventType = "contextmenu";
            } else if (tools.eqs(event.type, "click")) {
                if (tools.eqs(target.tagName, "span") && target.getAttribute("treeNode" + consts.id.SWITCH) !== null) {
                    tId = tools.getNodeMainDom(target).id;
                    nodeEventType = "switchNode";
                } else if (target.className && target.className.indexOf("nodeDiv") != -1) {
                    var childs = target.childNodes;
                    var child = childs[0];
                    for (var index = 0; index < childs.length; index++) {
                        if (tools.eqs(childs[index].tagName, "a")) {
                            child = childs[index];
                            break;
                        }
                    }
                    tmp = tools.getMDom(setting, child, [{tagName: "a", attrName: "treeNode" + consts.id.A}]);
                    if (tmp) {
        ...
        view = {
            ...
            makeDOMNodeLine: function (html, setting, node) {
                html.push("<div class='nodeDiv' style='padding-left: " + (setting.view.paddingLeft * node.level+setting.view.initPaddingLeft) + "px' tree_node_div onclick='" + (node.click || '') + "'><span id='", node.tId, consts.id.SWITCH, "' title='' class='", " icon ", view.makeNodeLineClass(setting, node), "' treeNode", consts.id.SWITCH, "></span>");
            },
    ...
    var _consts = {
        ...
        _zTreeTools = function (setting, zTreeTools) {
            ...
            zTreeTools.moveNode = function (targetNode, node, moveType, isSilent) {
                ...
                var isParent = data.nodeIsParent(setting, targetNode);
                if(isParent){
                    $$(node, this.setting).children("div.nodeDiv").find("span.node_name").text(node.name);
                }
        ...
        _view = {
            ...
            addRemoveBtn: function (setting, node) {
                ...
                var aObj = $$(node, consts.id.A, setting),
                    removeStr = "<span class='" + consts.className.BUTTON + " remove bdp-icon bzzx-close' id='" + node.tId + consts.id.REMOVE + "' title='" + tools.apply(setting.edit.removeTitle, [setting.treeId, node], setting.edit.removeTitle) + "' treeNode" + consts.id.REMOVE + " style='display:none;'></span>";
            ...
            moveNode: function (setting, targetNode, node, moveType, animateFlag, isSilent) {
                ...
                if (target_ulObj.get(0) && moveType == consts.move.TYPE_INNER) {
                    var targetLevel = targetNode.level?(targetNode.level + 1):0;
                    $("div.nodeDiv", $(nodeDom)).css({"padding-left": (targetLevel * setting.view.paddingLeft+setting.view.initPaddingLeft)});
