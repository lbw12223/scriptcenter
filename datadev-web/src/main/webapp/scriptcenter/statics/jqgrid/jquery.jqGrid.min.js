        ...
        return this.each( function() {
            ...
            var sortkeys = ["shiftKey","altKey","ctrlKey"],
                ...
                addLocalData = function() {
                    ...
                    //设置data长度为当前页条数
                    if(ts.p.remoteSort!=null && ts.p.remoteSort==false){
                        ts.p.data.length = ts.p.reccount;
                    }
                    ...
                    var total;
                    //当前页排序不触发
                    if(ts.p.remoteSort!=undefined || ts.p.remoteSort==false) {
                        total = ts.p.records;
                    }else{
                    ...
                    //使用当前页排序不触发
                    if(ts.p.remoteSort == undefined || ts.p.remoteSort == true){
                ...
                sortData = function (index, idxcol,reload,sor, obj){
                    ...
                    //当前页排序
                    if(ts.p.remoteSort!=undefined && ts.p.remoteSort==false){
                        //loadComplete 中需要将rows数据赋值给ts.p.data
                        populate2();
                    }else
                        populate();
...
var jqgrid_time_clock;//定时刷新时间全局变量；
(function($){
    /**
    ...
    $.jgrid.extend({
        ...
        navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView) {
            o = $.extend({
                ...
                refreshtime:false,
                refreshtimeicon:'icon-time',
                refreshtimetext:'',
                refreshtimetitle:'定时刷新',
            ...
            return this.each(function() {
                ...
                for(i = 0; i<clone; i++) {
                    ...
                    if (o.refreshtime) {
                        tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
                        $(tbd).append("<div class='ui-pg-div'><span class='ui-icon "+o.refreshtimeicon+"'>"+o.refreshtimetext+"</span></div>");
                        $(tbd).append("<div class='ui-pg-div input-time' style='display:none'><span class='ui-icon' style='font-size:14px;'><input type='text' value='"+ o.refreshtimenumber+"' class='time-num' style='font-size:12px;width:40px;height: 20px;line-height: 20px' value='10'/> s </span></div>");
                        $("tr",navtbl).append(tbd);
                        $(tbd,navtbl)
                            .attr({"title":o.refreshtimetitle  || "",id: "refresh_"+elemids})
                            .click(function(){
                                $('.input-time',navtbl).show();
                                if (!$(this).hasClass('ui-state-disabled')) {
                                    if($.isFunction(o.beforeRefresh)) {o.beforeRefresh.call($t);}
                                    $t.p.search = false;
                                    if($.isFunction(o.afterRefresh)) {o.afterRefresh.call($t);}
                        ...
                        jqgrid_time_clock = setInterval(function() {//启动定时器
                            $($t).trigger("reloadGrid", [{current:true}]);
                        },parseInt($(".time-num",navtbl).val()) * 1000);

                        $(".time-num",navtbl).keydown(function (event) {
                            if (event.keyCode == 13) {
                                var time = $(this).val();
                                if(!$.isNumeric(time)) {
                                    alert("请输入数字！");
                                ...
                                time = parseInt(time);
                                if(time > 0) {//数字>0启动定时器
                                    clearInterval(jqgrid_time_clock);
                                    jqgrid_time_clock = setInterval(function() {
                                        $($t).trigger("reloadGrid", [{current:true}]);
                                    },time * 1000)
                                } else {//停止定时器
                                    clearInterval(jqgrid_time_clock);
                                }
                                $('.input-time',navtbl).hide();
                            }
                        });
                        tbd = null;
                    }
        ...
        expandRow: function (record){
            this.each(function(){
                ...
                $(childern).each(function(){
                    var ids  = $t.p.idPrefix + $.jgrid.getAccessor(this,$t.p.localReader.id);
                    //todo 收缩节点操作把子节点的数据清除，根据PID清除所有的子节点;
//                    console.debug(ids,$($($t).jqGrid('getGridRowById', ids)));
//                    $($($t).jqGrid('getGridRowById', ids)).css("display","");
                    if(this[expanded]) {
...
//                    $($($t).jqGrid('getGridRowById', id)).css("display","none");
                    //todo 收缩节点操作把子节点的数据清除，根据PID清除所有的子节点;
                    $($($t).jqGrid('getGridRowById', id)).remove();
                    if(this[expanded]){
...
//    $('.jqgfirstrow td:last-child').each(function() {
//        $(this).css("width",$(this).width() - 1);
//    })
//
//    $('.ui-jqgrid-labels th:last-child').each(function() {
//        $(this).css("width",$(this).width() - 1);
//    })
}


jqGrid.initWidth = function(a,t,p) {
    a(t).each(function() {
        // console.debug(a(p).parents('.bdp-hidden'));
        a(this).jqGrid("setGridWidth", a(this).parents(p).width());
    })
}

jqGrid.operationHover = function(tableId) {
    $(tableId).on("mouseover", "tr.ui-widget-content",function () {
        $(this).find(".title-bottom").css("display", "block");
        $(this).find('a [class^="icon-"]').css("display", "block");
        $(this).find(".opt-list").css("line-height", "20px");
    }).on("mouseout", "tr.ui-widget-content", function () {
        $(this).find(".title-bottom").hide();
        $(this).find('a [class^="icon-"]').css("display", "inline")
        $(this).find(".opt-list").css("line-height", "30px");
    });
}