;(function ($, window) {
    "use strict";
    var guid = 0, ignoredKeyCode = [9, 13, 17, 19, 20, 27, 33, 34, 35, 36, 37, 39, 44, 92, 113, 114, 115, 118, 119, 120, 122, 123, 144, 145], allowOptions = ['source', 'empty', 'limit', 'cache', 'focusOpen', 'selectFirst', 'changeWhenSelect', 'highlightMatches', 'ignoredKeyCode', 'customLabel', 'customValue', 'template', 'combine', 'callback'], userAgent = (window.navigator.userAgent || window.navigator.vendor || window.opera), isFirefox = /Firefox/i.test(userAgent), isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(userAgent), isFirefoxMobile = (isFirefox && isMobile), $body = null, localStorageKey = 'autocompleterCache', supportLocalStorage = (function () {
    ...
    function _build($node, opts) {
        if (!$node.hasClass("autocompleter-node")) {
            ...
            data.$node.on("keyup.autocompleter", data, _onKeyup).on("keydown.autocompleter", data, _onKeydownHelper).on("focus.autocompleter", data, _onFocus).on("blur.autocompleter", data, _onBlur).on("mousedown.autocompleter", data, _onMousedown);
        }
    }
    ...
    function _launch(data) {
        ...
        if (!data.empty && data.query.length === 0) {
            ...
            if (typeof data.source === 'object') {
                ...
                var ajaxData = $.extend({limit: data.limit, query: data.query}, data.combine());
                data.jqxhr = $.ajax({
                    url: data.source,
                    dataType: "json",
                    type:"post",
            ...
            if (data.template) {
                ...
                for (var property in list[item]) {
                    if (list[item].hasOwnProperty(property)) {
                        ...
                        var attrValue = list[item][property] + "" ;
                        attrValue = attrValue.replace(new RegExp("<","gm"),"&lt;").replace(new RegExp(">","gm"),"&gt;");
                        //attrValue = attrValue.replace(new RegExp("<","gm"),"&lt;").replace(new RegExp(">","gm"),"&gt;");
                        //list[item][property] = list[item][property].replace(new RegExp("<","gm"),"&lt;").replace(new RegExp(">","gm"),"&gt;")
